...
begin
  ...
  gtk_signal_connect(PGtkObject(btn2), 'button-press-event', G_CALLBACK(@press1), nil);

  btn3 := gtk_button_new_with_label('Button3');
  gtk_fixed_put(GTK_FIXED(fixed), btn3, 100, 100);
  gtk_widget_set_size_request(btn3, 75, 25);
  gtk_signal_connect(PGtkObject(btn3), 'button-press-event', G_CALLBACK(@press1), nil);
