...
uses
  ...
  function CreateRadio(parent: PGtkWidget; var group: PGSList; szLabel: PChar): PGtkWidget;
  begin
  ...
  procedure main;
  var
    Window, vbox, radio, label_, rb: PGtkWidget;
  const
    group: PGSList = nil;
  begin
    GTK_Init(@argc, @argv);
    ...
    rb := gtk_radio_button_new_with_label(group, 'test');
    group := gtk_radio_button_group(GTK_RADIO_BUTTON(rb));
    gtk_container_add(GTK_CONTAINER(Window), rb);
